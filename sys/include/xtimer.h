...
/**
 * @defgroup  sys_xtimer System Timer
 * @ingroup   sys
 ...
 * time (last_wakeup + interval). last_wakeup is set to xtimer_now().
 *
 * This function can be used to create periodic wakeups.
 * @c last_wakeup should be set to xtimer_now() before first call of the function.
 *
 * If the result of (@c last_wakeup) would be in the past, the function sets
 * @c last_wakeup to xtimer_now() and returns immediately.
 *
 ...
 * the future.
 *
 * @warning BEWARE! Callbacks from xtimer_set() are being executed in interrupt context
 * (unless offset < XTIMER_BACKOFF). DON'T USE THIS FUNCTION unless you know
...
{
    xtimer_usleep64((uint64_t)seconds*1000000);
}
