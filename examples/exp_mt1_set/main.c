...
{
    (void)buf;
    mystats_inc_rx_app("get", (unsigned)_trans_id);
    _send_status(model, ctx);
}
                       ...
                       struct os_mbuf *buf)
{
    (void)model;
    (void)ctx;
    mystats_inc_rx_app("status", (unsigned)buf->om_data[0]);
...
static int _cmd_run(int argc, char **argv)
{
    ...
    for (unsigned i = 0; i < cnt; i++) {
        ...
        bt_mesh_model_msg_init(model->pub->msg, OP_GET);
...
int main(void)
{
    ...
    puts("ICN-BLE experiment: many-to-1 A");
