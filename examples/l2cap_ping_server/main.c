...
static void _on_data(struct ble_l2cap_event *event)
{
    ...
    printf("[nim] RX: got %i bytes\n", (int)rx_len);
    assert(rx_len <= MTUSIZE);
...
static int _cmd_connect(int argc, char **argv)
{
    ...
    int res = ble_gap_connect(nimble_riot_own_addr_type, &e->addr, (int32_t)CONN_WAIT,
...
int main(void)
{
    ...
    int res = ble_l2cap_create_server(COC_CID, MTUSIZE,
