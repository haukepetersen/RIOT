...
#include "tests-gorm_gatt_server.h"
...
Test *tests_gorm_gatt_tab_tests(void)
{
    EMB_UNIT_TESTFIXTURES(fixtures) {
        new_TestFixture(test_service_reg),
    };

    EMB_UNIT_TESTCALLER(gorm_gatt_tab_tests, NULL, NULL, fixtures);

    return (Test *)&gorm_gatt_tab_tests;
}

void tests_gorm_gatt_server(void)
{
    TESTS_RUN(tests_gorm_gatt_tab_tests());
}
