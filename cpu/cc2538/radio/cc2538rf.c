...
/**
 * @ingroup     cpu_cc2538rf
 * @{
 *
 * @file
 * @brief       Implementation of the radio driver for the CC2538
 *
...
/**
 * @brief   Default channel and frequency parameters
 * @{
 */
#define CHAN_MIN        (9U)
#define CHAN_MAX        (31U)
#define FREQ_MIN        (2405U)
#define FREQ_MAX        (2505U)
#define CHAN_SPACING    (5U)
/** @} */


void cc2538rf_setup(cc2538rf_t *dev)
{
    dev->driver = &cc2538rf_driver;
}

void cc2538rf_init(void)
{
...
uint16_t cc2538rf_get_channel(void)
{
    printf("current freq: %i\n", (int)RFCORE_XREG_FREQCTRL);

    printf("chan min %i, freq_min %i, chan_spading %i\n", (int)CHAN_MIN, (int)FREQ_MIN, (int)CHAN_SPACING);

    return (CHAN_MIN + ((RFCORE_XREG_FREQCTRL - FREQ_MIN) / CHAN_SPACING));
}

void cc2538_set_channel(uint16_t chan)
{
    if (chan > CHAN_MAX) {
        chan = CHAN_MAX;
    }
    ...
    RFCORE_XREG_FREQCTRL = (FREQ_MIN + (chan * CHAN_SPACING));
}
