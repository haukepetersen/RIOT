...
static void _cmd(dfplayer_t *dev, uint8_t cmd, uint16_t param)
{
    ...
    if ((dev->rx_pos == POS_START) && (data == START_BYTE)) {
        ...
        if (data == END_BYTE) {
            ...
            if (csum == _csum(dev->rx_buf.raw)) {
                memcpy(dev->rx_pkt.raw, dev->rx_buf.raw, DFPLAYER_PKTLEN);
                event_post(&_q, &dev->event.super);
            }
            else {
                DEBUG("CSUM wrong\n");
            }
...
{
    dfplayer_t *dev = ((dfplayer_event_t *)arg)->dev;

    DEBUG("[dfplayer] _on_pkt: got pkt from device:\n");
    _dump(dev->rx_pkt.raw);
}
...
int dfplayer_init(dfplayer_t *dev, const dfplayer_params_t *params)
{
    ...
    /* setup UART receiver */
    dev->rx_buf.pkt.start = START_BYTE;
    dev->rx_buf.pkt.end = END_BYTE;
    dev->rx_pos = 0;
    dev->event.super.handler = _on_pkt;
...
{
    assert(dev);
    _cmd(dev, CMD_WAKEUP, 0);
}

void dfplayer_playback(dfplayer_t *dev)
{
    assert(dev);
    _cmd(dev, CMD_PLAYBACK, 0);
}
