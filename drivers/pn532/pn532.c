...
static int _write(pn532_t *dev, char *buff, unsigned len)
{
    ...
    if (dev->mode == PN532_I2C) {
        ...
        reverse(buff, len);
        spi_transfer_byte(dev->conf->spi, GPIO_UNDEF, true, SPI_DATA_WRITE);
        spi_transfer_bytes(dev->conf->spi, GPIO_UNDEF, true, buff, NULL, len);
        gpio_set(dev->conf->nss);
...
static int _read(pn532_t *dev, char *buff, unsigned len)
{
    ...
    if (dev->mode == PN532_I2C) {
        ...
        spi_acquire(dev->conf->spi, GPIO_UNDEF, SPI_MODE, SPI_CLK);
        gpio_clear(dev->conf->nss);
        spi_transfer_byte(dev->conf->spi, GPIO_UNDEF, true, SPI_DATA_READ);
        spi_transfer_bytes(dev->conf->spi, GPIO_UNDEF, true, NULL, &buff[1], len);
        gpio_set(dev->conf->nss);
        spi_release(dev->conf->spi);
