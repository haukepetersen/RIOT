                                   ...
                                   ng_netif_hdr_t *hdr)
{
    ...
    if (hdr->dst_l2addr_len == 2) {
        ...
        buf[pos++] = dst_addr[0];
        /* set source address */
        buf[pos++] = (uint8_t)((dev->addr_short) >> 8);
        buf[pos++] = (uint8_t)((dev->addr_short) & 0xff);
    }
...
size_t _get_frame_hdr_len(uint8_t *mhr)
{
    uint8_t tmp;
    size_t len = 3;
...
static int _set_state(ng_at86rf2xx_t *dev, ng_netconf_state_t state)
{
    switch (state) {
        case NETCONF_STATE_SLEEP:
            ng_at86rf2xx_set_state(dev, STATE_TRX_OFF);
            break;
...
{
    ng_at86rf2xx_state_t state = ng_at86rf2xx_get_state(dev);
    switch (state) {
        case STATE_SLEEP:
...
static int _get(ng_netdev_t *device, ng_netconf_opt_t opt, void *val, size_t max_len)
{
    if (device == NULL) {
        return -ENODEV;
    }
    ...
    switch (opt) {
        ...
        case NETCONF_OPT_PROMISCUOUSMODE:
            ...
            if (dev->options & OPT_PROMISCUOUS) {
                return NETCONF_ENABLE;
            }
            else {
                return NETCONF_DISABLE;
...
static int _set(ng_netdev_t *device, ng_netconf_opt_t opt, void *val, size_t len)
{
    ng_at86rf2xx_t *dev = (ng_at86rf2xx_t *) device;
    ...
    switch (opt) {
        ...
        case NETCONF_OPT_AUTOACK:
            ...
            ng_at86rf2xx_set_option(dev, OPT_AUTOACK, ((bool *)val)[0]);
        ...
        case NETCONF_OPT_PRELOADING:
            ...
            ng_at86rf2xx_set_option(dev, OPT_PRELOADING, ((bool *)val)[0]);
            return sizeof(ng_netconf_enable_t);
...
static void _isr_event(ng_netdev_t *device, uint32_t event_type)
{
    ...
    if (irq_mask & NG_AT86RF2XX_IRQ_STATUS_MASK__RX_START) {
        if (dev->event_cb && (dev->options & OPT_TELL_RX_START)) {
    ...
    if (irq_mask & NG_AT86RF2XX_IRQ_STATUS_MASK__TRX_END) {
        ...
        else if (state == STATE_TX_ARET_ON) {
            if (dev->event_cb && (dev->options & OPT_TELL_TX_END)) {
