...
void ng_at86rf2xx_reset(ng_at86rf2xx_t *dev)
{
    ...
    /* in cases cpuid len < 8, we just add random memory content */
    memcpy(&addr_long, tmp, 8);
    ng_at86rf2xx_set_addr_long(dev, addr_long);
    /* now compress the long addr to form the short address */
    for (int i = 2; i < 8; i++) {
        tmp[i & 0x01] ^= tmp[i];
    }
    memcpy(&addr_short, tmp, 2);
    ...
    /* set default options */
    ng_at86rf2xx_set_option(dev, OPT_AUTOACK, true);
    ng_at86rf2xx_set_option(dev, OPT_CSMA, true);
    ng_at86rf2xx_set_option(dev, OPT_TELL_RX_END, true);
...
{
    ng_at86rf2xx_state_t state;

    /* make sure ongoing transmissions are finished */
    do {
        state = ng_at86rf2xx_get_state(dev);
    }
    while (state == STATE_BUSY_RX_AACK);
    dev->idle_state = state;
    ng_at86rf2xx_set_state(dev, STATE_TX_ARET_ON);
...
void ng_at86rf2xx_tx_exec(ng_at86rf2xx_t *dev)
{
    ...
    if (dev->event_cb && (dev->options & OPT_TELL_TX_START)) {
